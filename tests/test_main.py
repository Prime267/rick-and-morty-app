...
def test_full_sync_and_get_flow(client, mocker):
    ...
    page1_data = {
        ...
        "results": [
            ...
            # FIX: Wrapped dict for E501
            {"id": 1, "name": "Rick Sanchez", "species": "Human",
             "status": "Alive", "origin": {"name": "Earth (C-137)"}},
            # 2. Invalid: Species
            # FIX: Wrapped dict for E501
            {"id": 2, "name": "Alien Rick", "species": "Alien",
             "status": "Alive", "origin": {"name": "Earth (C-137)"}},
            # 3. Invalid: Status
            # FIX: Wrapped dict for E501
            {"id": 3, "name": "Dead Rick", "species": "Human",
             "status": "Dead", "origin": {"name": "Earth (C-137)"}},
        ]
    }
    ...
    page2_data = {
        ...
        "results": [
            # 4. Invalid: Origin
            # FIX: Wrapped dict for E501
            {"id": 4, "name": "Mars Rick", "species": "Human",
             "status": "Alive", "origin": {"name": "Mars"}},
            # 5. Valid: Human, Alive, Earth (another variation)
            # FIX: Wrapped dict for E501
            {"id": 5, "name": "Morty Smith", "species": "Human",
             "status": "Alive",
    ...
    # FIX: Wrapped long string for E501
    assert response_sync.json() == {
        "message": "Data synced successfully: 2 characters processed"
    }
...
def test_sync_upsert_logic(client, mocker):
    ...
    initial_data = {
        ...
        "results": [
            # FIX: Wrapped dict for E501
            {"id": 1, "name": "Rick", "species": "Human", "status": "Alive",
    ...
    updated_data = {
        ...
        "results": [
            # FIX: Wrapped dict for E501
            {"id": 1, "name": "Old Rick", "species": "Human", "status": "Alive",
             "origin": {"name": "Earth (C-137)"}}
        ]
    }
